				...
				<nav className="mx-auto max-w-6xl flex items-center justify-between h-14 px-4 sm:px-5 rounded-2xl border border-border/50 bg-background/70 backdrop-blur-xl shadow-sm transition-all duration-200 hover:border-border/80">
					...
					<div className="flex-1 max-w-sm mx-4 relative">
						...
						{searchOpen && query.trim() && (
							<>
								<div className="fixed inset-0 z-40" onClick={() => { setSearchOpen(false); setQuery(""); }} />
											...
											{results.map((item) => (
												...
												>
													<div className="h-7 w-7 rounded-lg bg-white/5 flex items-center justify-center shrink-0">
														{item.type === "project" ? (
															<svg className="h-3.5 w-3.5 text-[#ff6a00]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
														) : (
															<svg className="h-3.5 w-3.5 text-[#a0a0a0]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
														)}
													</div>
													<div className="min-w-0">
														<p className="text-sm font-medium text-white truncate">{item.label}</p>
														<p className="text-xs text-[#606060] truncate">{item.sub}</p>
													</div>
												</button>
											))}
										</div>
